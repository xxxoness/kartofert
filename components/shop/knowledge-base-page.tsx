...
export function KnowledgeBasePage() {
  ...
  const visibleFeatured = activeCategory === "Все статьи" || featuredArticle.filterCategory === activeCategory;
  const filteredArticles = useMemo(() => {
    if (activeCategory === "Все статьи") return articleCards;
    return articleCards.filter((article) => article.filterCategory === activeCategory);
  }, [activeCategory]);
      ...
      <section className="container-shell pb-8 md:pb-12">
        {filteredArticles.length === 0 && !visibleFeatured ? (
          ...
          <div className={cn("grid gap-5", visibleFeatured && "xl:grid-cols-[1.05fr_2fr]")}>
            {visibleFeatured && <FeaturedCard article={featuredArticle} />}
